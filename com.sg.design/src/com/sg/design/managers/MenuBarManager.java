 ...
 ******************************************************************************/
package com.sg.design.managers;
...
public class MenuBarManager extends MenuManager {
  ...
  public void fill( final Composite parent ) {
    ...
    toolbar.setData( RWT.CUSTOM_VARIANT, ICSSConstants.MENU_BAR_VARIANT );
    update( false, false );
  }
  ...
  private void makeEntry( final IContributionItem item ) {
    ...
    if( tempItem != null && tempItem instanceof MenuManager ) {
      ...
      toolItem.setData( RWT.CUSTOM_VARIANT, ICSSConstants.MENU_BAR_VARIANT );
  ...
  private void createMenu( final MenuManager manager, final ToolItem toolItem )
  {
    ...
    menu.setData( RWT.CUSTOM_VARIANT, ICSSConstants.MENU_BAR_VARIANT );      
  ...
  private void styleMenuItems( final Menu menu ) {
    ...
    if( items != null && items.length > 0 ) {
      for( int i = 0; i < items.length; i++ ) {
        items[ i ].setData( RWT.CUSTOM_VARIANT, ICSSConstants.MENU_BAR_VARIANT );
        Menu subMenu = items[ i ].getMenu();
        if( subMenu != null ) {
          subMenu.setData( RWT.CUSTOM_VARIANT, ICSSConstants.MENU_BAR_VARIANT );
