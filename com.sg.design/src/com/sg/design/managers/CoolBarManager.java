...
//  };
  ...
  public Control createControl2( final Composite parent ) {
    ...
    toolbar.setData( RWT.CUSTOM_VARIANT, ICSSConstants.HEADER_TOOLBAR );
    ...
    if( ( isDirty() || force ) && getControl2() != null ) {
      ...
      while( visibleItemItr.hasNext() ) {
        ...
        StylingSelectionAdapter listener 
          = new StylingSelectionAdapter( ICSSConstants.HEADER_TOOLBAR );
  ...
  private void styleToolItems() {
    if( toolbar != null ) {
      ...
      for( int i = 0; i < items.length; i++ ) {
        ...
        toolItem.setData( RWT.CUSTOM_VARIANT, ICSSConstants.HEADER_TOOLBAR );
      }
    }
  }
